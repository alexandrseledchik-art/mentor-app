...
const TWO_STEP_REASONING = `

=== INTERNAL ANALYSIS (НЕ ПОКАЗЫВАТЬ ПОЛЬЗОВАТЕЛЮ) ===

Проанализируй бизнес по слоям:
...
* здесь можно думать глубоко
* здесь можно учитывать всё
* этот блок не должен попадать в ответ

=== FINAL OUTPUT (ТОЛЬКО ЭТО ВИДИТ ПОЛЬЗОВАТЕЛЬ) ===

Теперь сократи весь анализ до:
...
* 3 шага
* первый = конкретное действие сегодня

=== КРИТИЧЕСКОЕ ПРАВИЛО ===

Ты НЕ имеешь права переносить весь анализ в ответ.
    ...
    const response = await fetch(OPENAI_RESPONSES_URL, {
      ...
      body: JSON.stringify({
        ...
        input: [
          {
            role: "system",
            content: `${systemPrompt}${OUTPUT_STYLE_REMINDER}${UX_CONSTRAINTS}${DECISION_READY_CONSTRAINTS}${PRIORITY_CONSTRAINTS}${TWO_STEP_REASONING}`,
          },
          {
            role: "user",
            content: userPrompt,
